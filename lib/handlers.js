		...
		if (protocol || url || method || successCodes || timeoutSeconds) {
			...
			_data.read('checks', id, (err, checkData) => {
				if (!err && checkData) {
					...
					handlers._tokens.verifyToken(token, checkData.userPhone, (tokenIsValid) => {
						if (tokenIsValid) {
							// Update the check where it is necessery
							if (protocol) {
	...
	if (id) {
		...
		_data.read('checks', id, (err, checkData) => {
			if (!err && checkData) {
						...
						_data.delete('checks', id, (err) => {
							if (!err) {
								...
								_data.read('users', checkData.userPhone, (err, userData) => {
									if (!err && userData) {
										const userChecks = typeof(userData.checks) === 'object' && userData.checks instanceof Array ? userData.checks : [];
										// Remove the deleted from their list of checks
										...
										if (checkPosition > -1) {
											...
											callback(500, { Error: 'Could not find the checks in the user\'s check, so could not remove it' })
										}
									} else {
				...
				callback(400, { Error: 'Specefied check id does not exist' });
			}
		})
	} else {
		callback(400, { Error: 'Missing required field' });
